...
class FullAttention(nn.Module):
    def __init__(self, mask_flag=True, factor=4, scale=None, attention_dropout=0.1, output_attention=False):
...
class CrossAttentionLayer(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        ...
        time_in = rearrange(x, 'b ts_d (seg_num seg_d_model) -> (b ts_d) seg_num seg_d_model', seg_num=self.seg_num)
        time_enc, attn = self.time_attention(
            time_in, time_in, time_in, attn_mask
        )
        ...
        # Cross-Variate Phase
        dim_send = rearrange(dim_in, '(b ts_d) seg_num seg_d_model -> (b seg_num) ts_d seg_d_model', b=batch)
        batch_router = repeat(self.router, 'seg_num factor seg_d_model -> (repeat seg_num) factor seg_d_model',
                              repeat=batch)
        dim_buffer, _ = self.dim_sender(batch_router, dim_send, dim_send)
        dim_receive, A = self.dim_receiver(dim_send, dim_buffer, dim_buffer)
        dim_enc = dim_send + self.dropout(dim_receive)
        dim_enc = self.norm3(dim_enc)
        dim_enc = dim_enc + self.dropout(self.MLP2(dim_enc))
        dim_enc = self.norm4(dim_enc)

        final_out = rearrange(dim_enc, '(b seg_num) ts_d seg_d_model -> b ts_d (seg_num seg_d_model)', b=batch)
